...
import { getAllProjects } from "@/lib/community";
import { competitions } from "@/lib/competitions";

export const dynamic = "force-dynamic";

export default async function Stories() {
  const stories = await getAllProjects();
  ...
  return <>
    <section className="page-hero compact"><div className="shell">
      ...
      <p>这里集中展示审核通过的参赛复盘、获奖信息和项目文件。投稿成功后由维护团队审核，确认内容与文件安全后公开。</p>
    ...
    <section className="section"><div className="shell">
      ...
      </article>)}</div> : <div className="empty contribution-empty"><MessageCircle/><h2>还没有审核通过的公开分享</h2><p>投稿经维护团队审核通过后会显示在这里。</p><Link className="button" href="/contribute">成为第一位贡献者</Link></div>}
    </div></section>
  </>;
}
